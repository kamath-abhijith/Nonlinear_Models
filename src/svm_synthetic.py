...
SUPPORT VECTOR MACHINES ON SYNTHERIC 2D DATA
...
parser = argparse.ArgumentParser(
    description = "SUPPORT VECTOR MACHINES ON SYNTHETIC DATA"
)

parser.add_argument('--dataset', help="training and testing dataset", default='noise0')
parser.add_argument('--method', help="method of optimisation", default='rbf')
...
classifier = svm.SVC(C=param_C, kernel=method, degree=3)
...
ax = plt.gca()
save_res = path + 'samples_SVM_dataset_' + datafile + '_method_' + str(method) \
    + '_size_' + str(training_size)
...
plt.figure(figsize=(8,8))
ax = plt.gca()
save_res = path + 'accuracy_SVM_dataset_' + datafile + '_method_' + str(method) \
    + '_size_' + str(training_size)
